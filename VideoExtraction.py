...
def check_trained_model(trained_model):
    ...
    if os.path.isfile(trained_model):
        ...
        print("[Info] Download Sports1m pre-trained model: \"{}\"".format(' '.join(download_cmd)))
...
def run_C3D_extraction(feature_prototxt, ofile, feature_layer, trained_model):
    ...
    print("[Info] Running C3D feature extraction: \"{}\"".format(' '.join(feature_extraction_cmd)))
...
def get_features(feature_files, feature_layer):
    ...
    if isinstance(feature_files, np.basestring):
...
def main():
    ...
    video_file = 'SampleVideos/Explosion008_x264.mp4'
